...
class Installer {
    ...
    public function checkPhpConfig() {
        ...
        } else if (preg_match("/cli/", $result[0])) {
            $trouble = 1;
    ...
    public function composerInstall($first_pass = true) {
        ...
        if (empty(getenv('APP_ENV')) or getenv('APP_ENV') == "production") {
    ...
    protected function writeDotEnv($key, $value) { // edited from Laravel Framework @ KeyGenerateCommand.php
    ...
    public function checkMySQLConnection() {
        ...
        if ($this->runDbUser("SELECT 1;") !== false) {
    ...
    public function checkMySQLConfig() {
        ...
        ");
        $logbin = false;
        if (preg_match('/log_bin\s+(\w*)/', $mycnf, $matches) and $matches[1] == "ON") {
            $logbin = true;
        }
        if ($logbin and !(preg_match('/log_bin_trust_function_creators\s+(\w*)/', $mycnf, $matches) and $matches[1] == "ON")) {
        ...
        if ($trouble == 0) echo $this->c("All database configurations compatible!", "success");
    }
