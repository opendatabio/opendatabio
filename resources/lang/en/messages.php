...
return array (
  ...
  'andor' => 'and/or',
  'angle' => 'Angle',
  'plot_transect_azimuth' => 'Azimuth',
  'plot_transect_azimuth_hint' => 'For Transects and plots informed as POINT geometries, ODB will estimate the geometry using the informed dimensions and this azimuth as the direction, or orientation',
  'api_token' => 'API Token',
  ...
  'genus' => 'Genus',
  'genera' =>'Genera',
  'geom_duplicate' => 'The exact geometry is already registered in the database',
  ...
  'geom_other_parents_error' => 'The coordinates for this location do not fall within one ore more of the specified Other Parents geometries!',
  'geometry' => 'Geometry',
  'geom' => 'Geometry',
  ...
  'import_taxons_hint' => 'Each line in the file is a Taxon. For published taxon names you may only provide <code>name</code>. The API will check the external nomenclatural databases (Tropicos, IPNI, Mycobank, Zoobank and GBIF) and retrieve the relevant information (parent, reference, author) if a single match is found. If the name is unpublished, like a species morphotype, you must also provide a <code>person</code> as the author for such name. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-taxons">API docs</a> for a full list of columns and explanations.',
  'import_locations_hint' => '<strong>If table</strong> - each line is a location record and may have the following columns: <code>name**, adm_level**, parent, altitude, datum, geom** (or lat and long), uc, x, y, startx, starty, notes</code>. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-locations">API docs</a> for a full list and explanations.<br><br><strong>If geojson</strong> - must be a FeatureCollection <a href="https://geojson.org/">GeoJson format</a>, like those obtained from <a href="https://osm-boundaries.com/">OpenStreeMaps Boundaries</a>. Custom fields above may included in the "properties" tag of the geojson feature.',
  'import_trait_categories_file' => 'Translations & Categories File',
  'import_traits_hint' => 'Each line in the first file is a Variable and may have the following colunms: <code>export_name, type, objects, unit, range_min, range_max, value_length, link_type</code>, which may be mandatory depending on the Variable type. In addition, <code>name</code> and <code>description</code> for the variable  may be informed in this file for a single Language or Translation (the language with id=1).  For multiple translations use the translation file below.  See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-traits">API docs</a> for detailed explanation on the Post method.',
  ...
  'import_individuals_hint' => 'Each line in this file should be an Individual and the mandatory columns are: <code>collector, tag, dataset, date, and  location (or latitude, longitude)</code>. See   <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-individuals">API docs</a> for a full list of options and their possible values and value formats. "Biocollection" fields may be informed to also import Vouchers for the Individuals in the list.',
  'import_persons_hint' => 'Each line in this file should be a Person and possible columns are <code>full_name**, abbreviation,email,institution, biocollection, notes </code>. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-persons">API docs</a> for value formats',
  'import_individuallocations_hint' => 'Each line in this file should be a Location for an Individual. Columns needed <code>individual_id**</code>, [either <code>location</code> or <code>latitude</code> and <code>longitude</code>]**, <code>date_time</code>, <code>altitude</code>, <code>notes</code> and relative position fields: <code>x</code> and  <code>y</code> OR <code>angle</code> and <code>distance</code>. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-individual-locations">API docs</a> for value formats',
  'import_vouchers_hint' => 'Each line in this file should be a Voucher in a Biocollection for an Individual already registered. Possible columns are <code>individual**, biocollection**, biocollection_type, biocollection_number, collector, number, date, notes, dataset </code>. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-vouchers">API docs</a> for value formats. Note that Vouchers may be imported directly through an Individual import, but for vouchers having the same Collector and Number (i.e. Tag) as those of the Individual',
  'import_measurements_hint' => 'Each line must be a Measurement for one of the Core Objects: Individuals, Vouchers, Taxons or Locations. Possible columns are <code>dataset**, date**, object_type**, object_id**, person**, trait**, value**, link_id, bibreference, notes, duplicated</code>. See <a href="'.config('app.documentation_site_url').'docs/api/post-data/#post-measurements">API docs</a> for value formats.',
  ...
  'taxon_children' => 'Descendant taxons',
